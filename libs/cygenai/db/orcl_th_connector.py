...
class CyOracleThConnector(CyLangDB):
    def __init__(self, config: dict):
        self.__user = config["user"]
        self.__password = config["password"]
        self.__dsn = oracledb.makedsn(config["host"], config["port"], service_name=config["service_name"])

    def connect(self):
        
        try:
            self.__conn = oracledb.connect(user=self.__user, password=self.__password, dsn=self.__dsn)
